...
class Pipeline(nn.Module):
    ...
    def __init__(self, emb_target, net, device, batch_size=80, dim=112, edge=20, up=5, sym_part=1, gauss_amplitude=0.01, color=False, multistart=True):
        ...
        self.face = torch.zeros((batch_size,3,dim,dim,)).to(self.device)+ \
                                torch.rand((self.batch_size,1,self.dim,self.dim)).expand(self.batch_size,3,self.dim,self.dim).cuda()
        self.best_face = None
    ...
    def forward(self):
        batch_size = self.batch_size #80
        iters_before_restart = self.iters_before_restart #100
        if self.iters > iters_before_restart * self.N_restarts: # self.N_restarts : 10
            self.im_init = 0.994 * self.im_init # torch.Size([80, 3, 112, 112])
        
        face = self.face + self.im_init # torch.Size([80, 3, 112, 112])
        
        emb = self.resnet(face)
        
        cos_target = torch.cosine_similarity(emb,self.emb_target) # torch.Size([80, 512]) => # (80,)
        self.norm = torch.norm(emb[self.best_ind]).item() # 81.7
        norms = ((torch.norm(self.emb_target,dim=1) - torch.norm(emb,dim=1)) ** 2)
...
#         print(self.face.min().item(),self.face.max().item())
        self.face = torch.clamp(self.face,0,1)
#         self.face = (self.face/-0.5)/2
#         print(self.face.min().item(),self.face.max().item())
        if random.random() > 0.5:
            self.face = torch.flip(self.face, dims=(3,))
        return face[self.best_ind], cos_target[self.best_ind].item(), norms[self.best_ind] # best_cos.item()
